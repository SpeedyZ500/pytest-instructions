...
- [Coding Portion of Homework 1](#coding-portion-of-homework-1)
  ...
  - [11. Integration Tests and GitHub Classroom Autograding](#11-integration-tests-and-github-classroom-autograding)
    - [11.1 Viewing Results from GitHub Classroom Autograding](#111-viewing-results-from-github-classroom-autograding)
      - [**Option 1 – On GitHub.com**](#option-1--on-githubcom)
      - [**Option 2 – Inside VS Code** (if you installed the GitHub Actions extension in Project 0)](#option-2--inside-vs-code-if-you-installed-the-github-actions-extension-in-project-0)
...
You now have to tell VS Code that you want to use the virtual environment. This is called *activating* the environment. How you activate `venv` depends on your machine.
...
- a **positive test** provides evidence that the *code produces the correct result* for valid input because the test is based on a known math fact or expected behavior
- a **negative test** provides evidence that the *code does not produce an incorrect result*, and is used to detect logical errors or incorrect outcomes in your code
- a **type-checking test** confirms that a function you've written *raises the correct kind of error* when given input of the wrong type, and is used to "enforce contracts" so that your code only operates on the kinds of inputs you expect

In this assignment, we'll write type-checking tests, but later in the class we'll use the tool called `mypy` that helps us do *static type checking* so that we don't have to write so many tests. The tool `mypy` was one of the packages specified in `pyproject.toml`.

### 6.2 Positive Test

Suppose that we are given two sets *A = {1, 2}*  and *B = {2, 3}*. The union of two sets is the collection of all elements that are either in the first set or in the second or both. Thus, *A ∪ B = {1, 2, 3}*.
If we write a function called `union` that tries to implement the *union* set operation then it should output the set *{1,2,3}* if its inputs are set *A* and set *B*. This is the basis of the positive test.
...
3. The `test_union_positive` function defines the expected output based on the known behavior of the mathematical union operation, *A ∪ B = {1,2,3}*.
...
A negative test checks that the function **does not** return an incorrect result. Let's make up something incorrect by defining *A = {1}* and *B = {1, 2}*, and then noting that *A ∪ B ≠ {1}*.
...
Note that I chose this test to check whether I accidentally implemented the union as an intersection, since the intersection of *A* and *B* is *A ∩ B = {1}*
...
4. Open the most recent `run-autograding-tests` workflow run.  
5. Scroll to find the **Autograding Reporter** section.  
6. Expand it to see the detailed integration test results, including the table showing scores for each test.  The summary should look something like the following:
